...
market_averages AS (
    ...
    SELECT
        ...
        -- USD-normalized: safe for the API to blend ACROSS countries
        AVG(salary_midpoint_usd) AS market_avg_salary_usd
...
skill_salaries AS (
    ...
    SELECT
        ...
        -- USD-normalized: safe for the API to blend ACROSS countries
        AVG(jws.salary_midpoint_usd) AS avg_salary_with_skill_usd
...
SELECT 
    ...
    -- Skill salary stats (native currency — correct for single-country views)
    ss.jobs_with_skill,
    ...
    -- USD-normalized: use these when blending ACROSS countries (country=None)
    ROUND(ss.avg_salary_with_skill_usd::numeric, 2) AS avg_salary_with_skill_usd,
    ...
    ROUND(ma.market_avg_salary_usd::numeric, 2) AS market_avg_salary_usd,

    -- Premium calculation (native currency; percentage is currency-invariant
    -- since it's a ratio within this row's own country/currency)
    ROUND((ss.avg_salary_with_skill - ma.market_avg_salary)::numeric, 2) AS salary_premium_absolute,
    ROUND((ss.avg_salary_with_skill_usd - ma.market_avg_salary_usd)::numeric, 2) AS salary_premium_absolute_usd,
    ROUND(
